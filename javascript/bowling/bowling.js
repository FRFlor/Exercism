...
class Bowling {
	...
	isPreviousFrameStillInBaseRolls(currentFrameIndex) {
		return currentFrameIndex !== 0 &&
				this.frames[currentFrameIndex - 1].awaitsBaseRolls();
	}
}
...
class Frame {
	constructor() {
		this.rolls = [];
		this.remainingRolls = BASE_ROLLS_COUNT; // A frame by default may only have 2 rolls on it. This value may
		// change if strike/spare happens
	...
	tryInsertNewRoll(newRoll) {
		...
		switch (this.rolls.length) {
			...
			default:
				throw new Error(
						'CUSTOM: This frame contains more rolls than possible!');
		}
	}
